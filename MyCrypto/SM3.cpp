#include "SM3.h"
string sm3_hex(string);
SM3::SM3() :HashSuperClass() {}
SM3::SM3(string s) :HashSuperClass(s) {}
string SM3::hexdigest() {
	return sm3_hex(text);
}
 ...
 */
void _sm3(unsigned char* input, int ilen,
    unsigned char output[32])
{
    sm3_context ctx;
...
string sm3_hex(string s) {
    unsigned char output[32];
    _sm3((unsigned char*)(s.c_str()), s.size(), output);
    stringstream res;
    for (int i = 0; i < 32; ++i)
        res << hex << setfill('0') << setw(2) << int(output[i]);
    return res.str();
}
