#include "SHA512.h"
string sha512_hex(string);
SHA512::SHA512() :HashSuperClass() {}
SHA512::SHA512(string s) :HashSuperClass(s) {}
string SHA512::hexdigest() {
	return sha512_hex(text);
}
...
string sha512_hex(string s) {
    SHA512_CB sha512;
    unsigned char sha512Code[64];
    SHA512Init(&sha512);
    SHA512Update(&sha512, (unsigned char*)s.c_str(), strlen((char*)s.c_str()));
    SHA512Final(&sha512, sha512Code);
    stringstream ss;
    for (int i = 0; i < 64; ++i)
        ss << hex << setfill('0') << setw(2) << int(sha512Code[i]);
    return ss.str();
}
